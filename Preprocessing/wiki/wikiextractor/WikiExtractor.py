...
def load_templates(file, output_file=None):
    ...
    for line in file:
        ...
        category = categoryRE.findall(line)
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    for line in input:
        if '<' not in line:  # faster than doing re.search()
            if inText:
                page.append(line)
                # TODO  find category inside the main text
                cat = categoryRE.search(line)
                if cat:
                    category.append(cat.group(1))
        ...
        if tag == 'page':
            ...
            if (colon < 0 or (title[:colon] in acceptedNamespaces) and id != last_id and
                ...
                job = (id, revid, urlbase, title, page, category, ordinal)  # TODO
...
def extract_process(jobs_queue, output_queue, html_safe):
    ...
    while True:
        ...
        if job:
            ...
            Extractor(*job[:-1]).extract(out, html_safe)  # (id, urlbase, title, page)   # TODO: add category
            text = out.getvalue()
...
def main():
    ...
    if output_path != '-' and not os.path.isdir(output_path):
        try:
            ...
            logging.error('Couldoutput not create: %s', output_path)
            return
